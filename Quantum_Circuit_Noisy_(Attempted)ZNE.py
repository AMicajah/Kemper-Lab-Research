...
for i in range(timesteps):
    ...
    theta_list.append(2 * np.arcsin(np.sqrt(value_theta)))
    phi_list.append(2 * np.arcsin(np.sqrt(value_phi)))

shots = 1000

# ZNE Setup -------------------------------------------------------------------------------------------------------------------

def stretch_circuit(qc, scale_factor):
    ...
    for instr_tuple in qc.data:
        ...
        for _ in range(int(scale_factor)):
            qc_scaled.append(instr, qargs, cargs)
    return qc_scaled

def run_zne(qc, scale_factors=[3,6,9]):
    measured_probs = []
    for s in scale_factors:
        qc_scaled = stretch_circuit(qc, s)
        transpiled = transpile(qc_scaled, simulator)
        result = simulator.run(transpiled, shots=shots).result()
        counts = result.get_counts()
        p1 = counts.get("1", 0) / shots
        measured_probs.append(p1)

    coeffs = np.polyfit(scale_factors, measured_probs, 1)
    return coeffs[1] 

# sim with Noise and ZNE ----------------------------------------------------------------------------------------------------
probs_sim = []
...
for j in range(timesteps):
    ...
    qc.measure(1, 0)
    print(qc)
    print(stretch_circuit(qc,3))
    break
    p1_zne = run_zne(qc)
    probs_sim.append(p1_zne)
...
plt.plot(range(timesteps), probs_sim, "o-", markersize=4, linewidth=1, label="Qiskit sim (Noisy) (with ZNE)")
